...
for t=3:30
    ...
    for vInd=1:size(randVecs,1)
        ...
        for deltaInd=1:length(deltaVec)
            ...
            tailProbV(t,deltaInd,vInd)=real(1-1/pi*omegaDel*sum(omegaVal.*sin(eps*omegaVec)));
        end
        disp(['Tail Prob - calc- t=' num2str(t) ', vInd=' num2str(vInd)]);
    end
    
    for deltaInd=1:length(deltaVec)
        [max1,max2]=max(tailProbV(t,deltaInd,:));
        tailProbWorst(t,deltaInd)=max1;
        tailProbWorstVec(t,deltaInd)=max2;
        tailProbWorst(t,deltaInd)
    end    
    disp(['t= ' num2str(t) ' is done.']);
end

save resMain6run5_1400edges mVec randVecs tailProbWorst tailProbWorstVec tailProbV edges nodes deltaVec ;
